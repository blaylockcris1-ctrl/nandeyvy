...
function isPublic(l) {
  if (l.status === "pending_payment" || l.status === "pending_review") return false;
  if (l.status === "live") return true;
  return !l.mine;
}
function allListings() { return [...mine(), ...LISTINGS].filter(isPublic); }
function adminPin() { return sessionStorage.getItem("ny_admin") || ""; }
...
async function registerUser() {
  ...
  if (!name || phone.length < 8 || pin.length < 4) return alert("Nombre, teléfono Paraguay (595…) y PIN de 4 dígitos.");
  const u = { name, phone, pin, role, email };
  const res = await api("/api/users", u);
  if (!res) return alert("Ese teléfono ya existe o no hay servidor. Probá entrar.");
  const list = users();
  if (!list.some((x) => x.phone === phone)) list.push(u);
  localStorage.setItem("ny_users", JSON.stringify(list));
  REMOTE.users = list;
  setSession({ name, phone, role, email });
...
async function loginUser() {
  ...
  let u = await api("/api/login", { phone, pin });
  if (!u) u = users().find(x => x.phone === phone && x.pin === pin);
  if (!u) return alert("Teléfono o PIN incorrecto. Creá la cuenta abajo si es la primera vez.");
  setSession({ name: u.name, phone: u.phone, role: u.role, email: u.email || "" });
  location.hash = "#/";
  render();
}
function saveApiUrl() {
  ...
  return `
    ...
    <section class="story">
      <div class="wrap story-grid">
        <div>
          <p class="kicker">Por qué existimos</p>
          <h2>El lote ya está. Falta quien lo muestre bien.</h2>
          <p>En Paraguay se vende por WhatsApp, por un primo, por un aviso que se pierde a la semana. El dueño se cansa. El comprador no confía. La inmobiliaria de barrio no llega a Encarnación y CDE el mismo día.</p>
          <p>Ñande Yvy — <em>nuestra tierra</em> — es el lugar donde el aviso vive con foto, precio en dólares y en guaraníes, y un humano que responde.</p>
...
function publish() {
  if (!session()) {
    return `<div class="wizard">
      <h2>Creá una cuenta para publicar</h2>
      <p>El aviso queda atado a tu teléfono. Después pagás y un admin lo aprueba.</p>
      <a class="btn btn-dark" href="#/cuenta">Entrar o crear cuenta</a>
    </div>`;
  }
  return `
    <form class="wizard" onsubmit="event.preventDefault(); saveListing();">
      <h2>Publicar</h2>
      <p class="meta" style="margin-bottom:16px">Sesión: ${session().name}. <a href="#/planes">Planes</a>.</p>
      <div class="plans">
        ...
        <label class="plan"><input type="radio" name="p-who" value="agency">
          ...
          <span>Hasta 30 avisos y 8 agentes.</span></label>
        <label class="plan"><input type="radio" name="p-who" value="broker">
          <strong>Que venda Ñande Yvy · 5%</strong>
          <span>Nosotros hablamos con el comprador.</span></label>
      </div>
      ...
      <p class="meta" id="fx-hint">El aviso muestra guaraníes primero (ref. Gs. 7.300 = USD 1).</p>
      <div class="row2">
...
function officeView() {
  ...
  return `<div class="wizard">
    <h2>Oficina</h2>
    <p class="meta" style="margin-bottom:14px">Plan Oficina Gs. 357.700 / mes.</p>
    ...
    <div style="margin:12px 0">${o.agents.map(a => `
      <div class="plan-card" style="margin-bottom:8px">
        <p class="plan-name">${a.name}</p>
        <p class="plan-desc">${a.wa} · <a href="#" onclick="event.preventDefault();removeAgent('${a.id}')">Quitar</a></p>
...
function myAds() {
  ...
  return `<div class="wrap" style="padding:24px 0 80px">
    <h2>Mis avisos</h2>
    <div class="grid">${rows.map(l => `<div>${cardHTML(l)}<button class="btn btn-line" style="margin:8px 0 16px" type="button" onclick="deleteListing('${l.id}')">Borrar</button></div>`).join("") || "<p>Todavía no publicaste.</p>"}</div>
  </div>`;
}
function savedView() {
...
function nosotros() {
  return `<div class="wrap about">
    ...
    <p class="lead">Nació de una idea simple: en este país la tierra es historia de familia. Un lugar en castellano, con precio en guaraníes y en dólares, y alguien del otro lado del WhatsApp.</p>
    <div class="about-block">
      <h2>Al que vende</h2>
      <p>El aviso suelto se llena de “¿sigue disponible?”. El cartel en la reja se destiñe. Acá el aviso tiene foto, barrio, metros y un número que responde. Si no querés atender más, nos encargás la casa: cobramos 5% solo al firmar.</p>
    </div>
...
function planes() {
  return `<div class="wrap" style="padding:24px 0 80px;max-width:720px">
    ...
    <div class="plan-list">
      <div class="plan-card"><p class="plan-name">Dueño</p><p class="plan-desc">Gs. 87.600 (USD 12) · 30 días · 1 aviso · WhatsApp · 0% comisión</p></div>
      <div class="plan-card"><p class="plan-name">Oficina Starter</p><p class="plan-desc">Gs. 211.700 (USD 29) / mes · 12 avisos · 3 agentes</p></div>
      <div class="plan-card"><p class="plan-name">Oficina</p><p class="plan-desc">Gs. 357.700 (USD 49) / mes · 30 avisos · 8 agentes</p></div>
      <div class="plan-card"><p class="plan-name">Ñande Yvy vende</p><p class="plan-desc">5% al cierre (venta) o 1 mes de alquiler · nosotros atendemos</p></div>
    </div>
    <p class="meta">Oficina nueva: 5 avisos gratis por 30 días.</p>
...
function account() {
  ...
  return `<div class="wizard">
    ...
    <p class="meta">Dueño, inmobiliaria o comprador. El teléfono es el usuario. Sirve en Paraguay y en EE.UU.</p>
    <div class="field"><label>Nombre</label><input id="r-name" placeholder="Ana Benítez"></div>
    <div class="field"><label>Email</label><input id="r-email" type="email" placeholder="ana@correo.com"></div>
    <div class="field"><label>Teléfono (WhatsApp)</label><input id="r-phone" placeholder="595981123456"></div>
...
async function confirmPay() {
  const id = sessionStorage.getItem("ny_pending_id");
  if (id) await api("/api/listings/paid", { id });
  const rows = mine().map((x) => x.id === id ? { ...x, status: "pending_review" } : x);
  localStorage.setItem("ny_mine", JSON.stringify(rows));
  alert("Pago anotado. El aviso queda en revisión hasta que un admin lo apruebe.");
  location.hash = "#/mis";
}
function adminView() {
  const pin = adminPin();
  const rows = (REMOTE.listings || []).concat(mine()).filter((x, i, a) => a.findIndex((y) => y.id === x.id) === i);
  if (!pin) {
    return `<div class="wizard">
      <h2>Admin Ñande Yvy</h2>
      <p class="meta">PIN que cargás en Render como ADMIN_PIN.</p>
      <div class="field"><label>PIN</label><input id="ad-pin" type="password"></div>
      <button class="btn btn-dark" type="button" onclick="(async()=>{const p=$('#ad-pin').value;const r=await api('/api/admin/login',{pin:p});if(!r||!r.ok)return alert('PIN incorrecto');sessionStorage.setItem('ny_admin',p);render();})()">Entrar</button>
    </div>`;
  }
  return `<div class="wrap" style="padding:24px 0 80px">
    <h2>Admin</h2>
    <p class="meta">${rows.length} avisos en el servidor</p>
    ${rows.map((l) => `<div class="plan" style="margin:10px 0">
      <strong>${l.title || l.id}</strong>
      <span>${l.status || "sin estado"} · ${l.city || ""} · ${l.id}</span>
      <span>
        <a href="#/aviso/${l.id}">Ver</a>
        · <a href="#" onclick="event.preventDefault();adminAct('approve','${l.id}')">Aprobar</a>
        · <a href="#" onclick="event.preventDefault();adminAct('delete','${l.id}')">Borrar</a>
      </span>
    </div>`).join("") || "<p>No hay avisos.</p>"}
    <button class="btn btn-line" type="button" onclick="sessionStorage.removeItem('ny_admin');render()">Salir admin</button>
  </div>`;
}
async function adminAct(act, id) {
  const pin = adminPin();
  const r = await api("/api/admin/" + act, { id, pin });
  if (!r || !r.ok) return alert("No se pudo.");
  await boot();
  location.hash = "#/admin";
}
...
function payView() {
  ...
  return `<div class="wizard">
    ...
    <button class="btn btn-dark" type="button" onclick="confirmPay()">Ya pagué · enviar a revisión</button>
...
function saveListing() {
  ...
  sessionStorage.setItem("ny_pending_id", item.id);
  startPay(item.plan === "oficina49" ? "oficina49" : "usd12");
  };
...
function render() {
  ...
  else if (parts[0] === "admin") html = adminView();
  else html = home();
  $("#app").innerHTML = html;
